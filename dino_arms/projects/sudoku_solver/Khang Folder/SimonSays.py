    ...
    def player_follow_simon_cmd(self, difficulty):
        ...
        if self.simon_or_naw and self.current_act:
            ...
            if chance >= difficulty:
                ...
                self.current_act = None
                statement = random.choice(["Haha, you can't fool me", "Gotcha ya, enough playing around",
                                           "Get wrecked, I know what you're doing", "Can't fool me"])
                self.say_pub.publish(statement)
                time.sleep(2)
...
class AutonomousSimon(SimonSays):
    ...
    def run(self):
        ...
        statement = random.choice(
            ["I am going to demo Simon Says.", "Watch me play Simon Says", "Simon Says is cool! Watch this"])
        self.say_pub.publish(statement)
        time.sleep(2)
...
class EdwinSimon(SimonSays):
    ...
    def run(self):
        ...
        time.sleep(1)
        statement = random.choice(
            ["Alright, I will be Simon. Ready? Set? Let's play!", "Okay I'm Simon. Are you ready? Let's go!",
             "Who's Simon? I'm Simon. Hold on to your socks. Here we go!"])
        self.say_pub.publish(statement)
        time.sleep(6)
        ...
        if not self.no_mistakes:
            statement = random.choice(["Nice try, but you messed up. Game over.", "Get wrecked, baby, it's over.",
                                       "Hahahahah, I'm too quick for you! Sorry, but you lost"])
            move = "gloat"
        else:
            statement = random.choice(["Congratulations, you finished the game!", "Nice job, you completed my game!",
                                       "Wow so good, you're done without a sweat!"])
            move = "praise"
...
class EdwinPlayer(SimonSays):
    ...
    def run(self):
        ...
        self.check_completion()
        time.sleep(1)
        statement = random.choice(["Alright, I will be the player. Ready? Set? Let's play!",
                                   "I'm the player. Gimme your best shot! Let's go!",
                                   "Okay okay I'll be the player. I'm ready when you are."])
        self.say_pub.publish(statement)
        time.sleep(2)
        ...
        if not self.no_mistakes:
            statement = random.choice(["Oh no, I messed up. Good game though.", "Darn, I'm bad. Nice game.",
                                       "Ugh, a mistake! Excuse me while I go cry in a corner..."])
            move = "sad"
        else:
            statement = random.choice(["Yay! We made it to the end of the game. It was fun!", "Oh yes! I finished woo!",
                                       "Ohohohohoho baby I'm so good! I did it! Good game."])
            move = "praise"
