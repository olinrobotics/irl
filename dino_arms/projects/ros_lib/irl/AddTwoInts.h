...
static const char ADDTWOINTS[] = "irl/AddTwoInts";
  ...
  class AddTwoIntsResponse : public ros::Msg
  {
    ...
    {
      int offset = 0;
      uint32_t length_sum = strlen(this->sum);
      varToArr(outbuffer + offset, length_sum);
      offset += 4;
      memcpy(outbuffer + offset, this->sum, length_sum);
      offset += length_sum;
      return offset;
    }
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      int offset = 0;
      uint32_t length_sum;
      arrToVar(length_sum, (inbuffer + offset));
      offset += 4;
      for(unsigned int k= offset; k< offset+length_sum; ++k){
          inbuffer[k-1]=inbuffer[k];
      }
      inbuffer[offset+length_sum-1]=0;
      this->sum = (char *)(inbuffer + offset-1);
      offset += length_sum;
     return offset;
    }

    const char * getType(){ return ADDTWOINTS; };
    const char * getMD5(){ return "3c5c2c5628b5ddd7119bd2a006302ca9"; };
